...
**Working Title:** Pato em Fradelos (alt. candidates: *Sleep Fracture*)
**Genre:** 2D side-scroller platformer with puzzle and exploration elements
**Tone:** Grounded and realistic *except* for Black Matter, which is intentionally "alien" and physics-breaking — this contrast is deliberate and central to the feel
...
- **Key rule:** What matters for Black Matter observation is what appears on the *player's screen*, not what the duck character "sees" in-world. This enables perspective-based puzzles.
...
- When it "vanishes," it goes *somewhere* — likely another dimension (see Teleporter / Other Dimension section)
...
### The Observer
A perfect black sphere — literally just a giant eye. Single eye. No limbs. Can float inside Black Matter. Invisible when eye is closed; visible when eye is open. Located in the Aquarium.
- **Mechanically:** Follows and stares at the duck. Continuous exposure causes radiation/visual distortion (red tint, spinning effect). ~4 seconds of unbroken eye contact = duck dies.
...
| Sleeping | Duck stops observing → can fall through BM platforms. Also possibly related to Internull access. |
...
- `unresolved.html` — Open questions and design TODOs *(internal use)*
...
- Why is observation tied to the *player's screen* rather than the duck's vision? (needs in-world justification or intentional ambiguity)
...
- Why does BM observation depend on player screen and not duck POV — intentional design with no in-world reason, or worth justifying?
